...
public class Renderer {
    ...
    private static final RenderPipeline renderPipeline = RenderPipelines.register(RenderPipeline.builder(RenderPipelines.DEBUG_FILLED_SNIPPET)
            .withLocation(Identifier.fromNamespaceAndPath(SkyTrack.MOD_ID, "pipeline/debug_render_pipeline"))
            .build()
    );
    ...
    public void renderAndDrawWaypoint(WorldRenderContext context, AABB box, float[] rgba) {
        renderWaypoint(context, box, rgba);
        drawFilledBox(Minecraft.getInstance(), renderPipeline);
    }
     ...
     */
    private void renderWaypoint(WorldRenderContext context, AABB box, float[] rgba) {
        ...
        renderFilledBox(matrices.last().pose(), buffer, box, rgba);

        matrices.popPose();
    }

    /**
     * Draw a filled box with the given dimensions and color in a buffer to be ready to be rendered
     * @param matrix The matrix
     * @param buffer The buffer
     * @param box The box coordinates
     * @param rgba The color parameters
     */
    private static void renderFilledBox(Matrix4f matrix, BufferBuilder buffer, AABB box, float[] rgba) {
    ...
    private void drawFilledBox(Minecraft client, @SuppressWarnings("SameParameterValue") RenderPipeline pipeline) {
        // Build the buffer
    ...
    private GpuBuffer upload(MeshData.DrawState drawParameters, VertexFormat format, MeshData builtBuffer) {
        ...
        if (vertexBuffer == null || vertexBuffer.size() < vertexBufferSize) {
            ...
            vertexBuffer = new MappableRingBuffer(() -> SkyTrack.MOD_ID + " example render pipeline", GpuBuffer.USAGE_VERTEX | GpuBuffer.USAGE_MAP_WRITE, vertexBufferSize);
        }
    ...
    private static void draw(Minecraft client, RenderPipeline pipeline, MeshData builtBuffer, MeshData.DrawState drawParameters, GpuBuffer vertices, VertexFormat format) {
        ...
        try (RenderPass renderPass = RenderSystem.getDevice()
                ...
                .createRenderPass(() -> SkyTrack.MOD_ID + " example render pipeline rendering", client.getMainRenderTarget().getColorTextureView(), OptionalInt.empty(), client.getMainRenderTarget().getDepthTextureView(), OptionalDouble.empty())) {
